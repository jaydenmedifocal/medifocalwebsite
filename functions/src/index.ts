...
export const sendContactEmail = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  ...
  try {
    ...
    const recipientEmail = functions.config().contact?.recipient || 'admin@medifocal.com';
    ...
    const mailOptions = {
      from: `"Medifocal Contact Form" <${functions.config().gmail?.user || process.env.GMAIL_USER}>`,
      replyTo: email,
      to: recipientEmail,
...
This email was sent from the Medifocal contact form.
    ...
    try {
      const confirmationMailOptions = {
        from: `"Medifocal" <${functions.config().gmail?.user || process.env.GMAIL_USER}>`,
        to: email,
 ...
 */
export const onCreateCustomer = functions.region('australia-southeast1').firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      if (userData?.stripeId || userData?.stripeCustomerId) {
      ...
      const customer = await stripe.customers.create({
        ...
        name: userData?.displayName || userData?.name || `${userData?.firstName || ''} ${userData?.lastName || ''}`.trim(),
 ...
 */
export const getStripeCustomerId = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  try {
    ...
    if (!stripeCustomerId) {
      ...
      if (!paymentsSnapshot.empty) {
        ...
        stripeCustomerId = paymentDoc.data()?.stripeId || paymentDoc.data()?.stripeCustomerId;
      }
    }
 ...
 */
export const createStripePortalSession = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        stripeCustomerId = userData?.stripeId || userData?.stripeCustomerId || null;
      }
    ...
    if (!stripeCustomerId) {
      try {
        ...
        const paymentsSnapshot = await paymentsRef.limit(1).get();
        if (!paymentsSnapshot.empty) {
          stripeCustomerId = paymentsSnapshot.docs[0].data()?.stripeId || paymentsSnapshot.docs[0].data()?.stripeCustomerId || null;
        }
    ...
    if (!stripeCustomerId) {
      ...
      const customerEmail = userData?.email || context.auth.token.email || undefined;
      ...
      const customer = await stripe.customers.create({
        email: customerEmail,
        name: userData?.displayName || userData?.name || `${userData?.firstName || ''} ${userData?.lastName || ''}`.trim() || undefined,
 ...
 */
export const getStripeSubscriptions = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    const userData = userDoc.data();
    let customerId = userData?.stripeId || userData?.stripeCustomerId;
 ...
 */
export const searchStripeCustomer = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!stripe) {
    throw new functions.https.HttpsError('failed-precondition', 'Stripe is not configured');
  }
 ...
 */
export const getStripeInvoices = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const getStripeCustomerBilling = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const createCheckoutSession = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!stripe) {
    throw new functions.https.HttpsError('failed-precondition', 'Stripe is not configured');
  }
  ...
  try {
    ...
    if (paymentMethod === 'invoice') {
      ...
      if (stripeCustomerId) {
        ...
        if (existingCustomers.data.length > 0) {
          ...
          if (customerDetails || shippingAddress) {
            ...
            if (customerDetails?.phone) updateData.phone = customerDetails.phone;
            if (shippingAddress) {
              updateData.shipping = {
          ...
          if (customerDetails?.name) customerData.name = customerDetails.name;
          if (customerDetails?.phone) customerData.phone = customerDetails.phone;
          if (shippingAddress) {
      ...
      await stripe.customers.update(customer.id, {
        ...
        } as any, // TypeScript types may not include this field yet
      });
    ...
    const sessionParams: Stripe.Checkout.SessionCreateParams = {
      ...
      payment_method_types: ['card', 'payto' as any, 'afterpay_clearpay', 'klarna'],
 ...
 */
export const updateStripeCustomerBilling = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const cancelStripeSubscription = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const reactivateStripeSubscription = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const getStripePaymentMethods = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    if (!customerId) {
      ...
      const paymentsSnapshot = await paymentsRef.limit(1).get();
      if (!paymentsSnapshot.empty) {
        customerId = paymentsSnapshot.docs[0].data()?.stripeCustomerId;
      }
    }
 ...
 */
export const deleteStripePaymentMethod = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const setDefaultStripePaymentMethod = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
export const syncAllCustomersToStripe = functions.region('australia-southeast1').https.onCall(async (data, context) => {
  ...
  const userData = userDoc.data();
  const isAdmin = userData?.email === 'admin@medifocal.com' || userData?.role === 'admin';
  ...
  try {
    ...
    for (const customerDoc of customersSnapshot.docs) {
      try {
        ...
        if (customerData?.stripeId || customerData?.stripeCustomerId) {
          skipped++;
          continue;
        }
        ...
        const customer = await stripe.customers.create({
          ...
          name: customerData?.displayName || customerData?.name || 
                `${customerData?.firstName || ''} ${customerData?.lastName || ''}`.trim() || undefined,
          phone: customerData?.phoneNumber || customerData?.phone,
          metadata: {
            firebaseUID: userId,
            practiceName: customerData?.practiceName || '',
          },
        });
 ...
 */
export const handleCustomerPortalWebhook = functions.region('australia-southeast1').https.onRequest(async (req, res) => {
  if (!stripe) {
  ...
  try {
    switch (event.type) {
      ...
      case 'billing_portal.configuration.updated': {
        const configuration = event.data.object as Stripe.BillingPortal.Configuration;
 ...
 */
async function handleTaxIdCreated(taxId: Stripe.TaxId) {
  try {
 ...
 */
async function handleTaxIdDeleted(taxId: Stripe.TaxId) {
  try {
    const customerId = typeof taxId.customer === 'string' 
      ? taxId.customer 
      : taxId.customer?.id;
    ...
    if (customerQuery.empty) {
      ...
      const userDoc = altQuery.docs[0];
      const userData = userDoc.data();
      const taxIds = userData?.taxIds || [];
      const updatedTaxIds = taxIds.filter((tid: any) => tid.id !== taxId.id);
 ...
 */
async function handleTaxIdUpdated(taxId: Stripe.TaxId) {
  try {
    const customerId = typeof taxId.customer === 'string' 
      ? taxId.customer 
      : taxId.customer?.id;
    ...
    if (customerQuery.empty) {
      ...
      const userData = userDoc.data();
      const taxIds = userData?.taxIds || [];
      const updatedTaxIds = taxIds.map((tid: any) => 
        tid.id === taxId.id 
          ? {
              ...tid,
              verification: taxId.verification,
              updated: admin.firestore.Timestamp.fromDate(new Date(taxId.created * 1000)),
            }
          : tid
      );
