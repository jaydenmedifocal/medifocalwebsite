...
const COLLECTIONS = {
  ...
  // CATEGORIES already defined above (line 33)
 ...
 * Based on fsm.location model from field-service-18.0
 */
export const createLocation = async (locationData: any): Promise<string> => {
  try {
    const locationsRef = collection(db, COLLECTIONS.LOCATIONS);
    
    const newLocation = {
      name: locationData.name || locationData.completeName || 'New Location',
      completeName: locationData.completeName || locationData.name,
      ...
      personIds: locationData.personIds || [],
      createdAt: Timestamp.now(),
      created_at: Timestamp.now(),
      updatedAt: Timestamp.now(),
      updated_at: Timestamp.now()
